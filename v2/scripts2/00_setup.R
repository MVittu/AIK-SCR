...
this_file <- tryCatch(normalizePath(sys.frame(1)$ofile, mustWork = FALSE), error = function(e) NA_character_)
scripts_dir <- if (!is.na(this_file)) dirname(this_file) else file.path(getwd(), "v2", "scripts2")
v2_dir <- normalizePath(file.path(scripts_dir, ".."), mustWork = FALSE)
project_root <- normalizePath(file.path(v2_dir, ".."), mustWork = FALSE)

if (!dir.exists(file.path(project_root, "Data"))) {
  project_root <- normalizePath(getwd(), mustWork = FALSE)
  v2_dir <- file.path(project_root, "v2")
  scripts_dir <- file.path(v2_dir, "scripts2")
}
